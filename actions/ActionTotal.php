	...
	public function access($page) {		
		...
		if(Usuario::valida($serial)==true){
			...
			if(Consulta::isValida($consulta, "binXML")==true){
				...
				print_r(Consulta::efetuarConsulta("http://173.203.71.192/webservice/search.php?serial=1NQ4F09LAQ7&chassi=".$chassi."&tipo=6"));
				//Gravame
				print_r(Consulta::efetuarConsulta("http://173.203.71.192/webservice/search.php?serial=1NQ4F09LAQ7&chassi=".$chassi."&tipo=7"));
				//Sinistro
				...
				Logs::salvarConsulta("agile_consultas_total", $serial, 2);
			}
		}else{
